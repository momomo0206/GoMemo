// Package auth provides authentication and authorization for the GoMemo server.
//
...
import (
	...
	"github.com/momomo0206/gomemo/internal/util"
	"github.com/pkg/errors"
)

const (
	// Issuer is the issuer claim in JWT tokens.
	// This identifies tokens as issued by GoMemo.
	Issuer = "gomemo"
	...
	RefreshTokenCookieName = "gomemo_refresh"

	// PersonalAccessTokenPrefix is the prefix for PAT hashes.
	PersonalAccessTokenPrefix = "gomemo_pat_"
)
...
// Claims: {"name": username, "iss": "gomemo", "aud": [audience], "sub": userID, "iat": now, "exp": expiry}
...
func verifyJWTKeyFunc(secret []byte) jwt.Keyfunc {
	return func(t *jwt.Token) (any, error) {
		...
		if !ok || kid != KeyID {
			return nil, errors.Errorf("unexpected key ID: %v", t.Header["kid"])
		}
		return secret, nil
	}
}
