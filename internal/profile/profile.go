...
type Profile struct {
	...
	Data string
	// DSN points to where gomemo stores its own data
	DSN string
	...
	Version string
	// InstanceURL is the url of your gomemo instance
	InstanceURL string
}
...
func (p *Profile) Validate() error {
	...
	if p.Mode == "prod" && p.Data == "" {
		if runtime.GOOS == "windows" {
			p.Data = filepath.Join(os.Getenv("ProgramData"), "gomemo")
	...
	if p.Driver == "sqlite" && p.DSN == "" {
		dbFile := fmt.Sprintf("gomemo_%s.db", p.Mode)
